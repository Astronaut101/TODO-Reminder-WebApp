...
REMEMBER: We are not limited to the pre-baked logic. We can override almost any part of the request-response cycle.

#### Lists and Items

__ListCreate__ and __ItemCreate__ both extend the class *CreateView*. ItemUpdate will extend the generic view class UpdateView, in which this is quite similar to CreateView, and you can use the same template for both base class Django Views. For CreateView, it can be a base class for any view designed to create objects.

The generic views know how to handle the *POST* request generated by the form on a successful submit action. The child classes need to be told which Model they're based on.

When we have defined our routes, the user can reach each of the new views with the appropriate data values set. In the todo_app/urls.py script, we will be adding the routes as new items in the urlpatterns array, naming them "list-add", "item-add", and "item-update".

#### New Views

For our new lists that we have created (e.g. ListCreate, ItemCreate, and ItemUpdate) views. The first one that we'll tackle is the form for creating a new list.

### Delete To-Do Lists and Items

In our *Delete* functionality, we will add links to the forms to allow the user to delete one item at a time, or even an entire list

#### Make DeleteView Subclasses

In the *DeleteView* class from the django.views.generic.edit.DeleteView, it displays a confirmation page and deletes an existing object. The given object will only be deleted if the request method is POST. If this view is fetched via GET, it will display a confirmation page that should contain a form POSTs to the same URL

### Define Deletion confirmations and URLS

We will be offering the user the __Delete__ option from the editing pages, we only need to create new templates for the corresponding confirmation pages. There's even a default name for these confirmation tempaltes: 'modelname'_confirm_delete.html. And if such a template exists, our classes derived from DeleteView will automatically render it when the associated form is submitted.

### Enable Deletions

We need to add a mechanism for our user to invoke functionalities in which they are able to delete the ToDo List Items and the ToDo List title.

## Final Thoughts

In this project, we have learned that Django's class-based views are designed to help us bootstrap an application in minimum time! They cover the majority of the use cases in their unmodified form. But because they're class-based, they're not restrictive. You can change almost anything you might wnat to change about view behaviour.

## Next Steps

* [ ] Styling the ToDo Web Application
* [ ] Adding an email notification alarm if the ToDo list item is about to expire
* [ ] Handling Task Completion
* [ ] Making it Multi-User
* [ ] Exploring more Class-Based Views
